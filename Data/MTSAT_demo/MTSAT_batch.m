...
Model = MTSAT; % Create model object
...
%% STEP |LOAD PROTOCOL ----------------------------------------------------
%  (3)	|- Respective command lines appear if required by MTSAT. 
% -------------------------------------------------------------------------

% MTSAT object needs 3 protocol field(s) to be assigned:
...
%  (4)	|- Respective command lines appear if required by MTSAT. 
% -------------------------------------------------------------------------
% MTSAT object needs 4 data input(s) to be assigned:
...
data.T1w=double(load_nii_data('PDw.nii.gz'));
% T1w.nii.gz contains [128  128   96] data.
data.PDw=double(load_nii_data('T1w.nii.gz'));
...
save -mat7-binary 'MTSAT_FitResultsOctave.mat' 'FitResults';

else % ---> If MATLAB 

qMRsaveModel(Model,'MTSAT.qMRLab.mat'); 
