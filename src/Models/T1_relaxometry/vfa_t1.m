    ...
    methods
       ...
       function FitResult = fit(obj,data)
            ...
            if length(unique(TR))>1
                ...
                [xopt, residue] = lsqcurvefit(@(x,Prot) equation(obj, addfixparameters(obj.st,x,fixedparam))',obj.st(~fixedparam),[],double(data.VFAData),double(obj.lb(~fixedparam)),double(obj.ub(~fixedparam)),optoptim);
                m0 = xopt(1);
                t1 = xopt(2);
            else
       ...
       function plotModel(obj,x,data)
           ...
           if length(unique(TR))==1
               ...
               if exist('data','var')
               plot3(TR,flipAngles,data.VFAData,'xb','MarkerSize',16);
               hold on
               end
               plot3(TR,flipAngles,Smodel,'r');
               hold off
               xlabel('Repetition Time [sec]','FontSize',12);
               ylabel('Flip Angle [deg]','FontSize',12);
