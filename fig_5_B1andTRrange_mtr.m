...
MTRs = zeros(length(TR_range), length(B1_range));

for ii=1:length(TR_range)
    for jj=1:length(B1_range)
            fa = protocol.fa*B1_range(jj)
            tr = TR_range(ii)/1000
            te = protocol.te/1000
            offset = protocol.offset
            mt_shape = protocol.mtshape
            mt_duration = protocol.mtduration/1000
            mt_angle = protocol.mtangle*B1_range(jj)
...
save("fig7.mat", "TR_range", "B1_range", "MTRs", "trueTR")
