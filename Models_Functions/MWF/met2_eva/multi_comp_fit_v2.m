function FitResult = multi_comp_fit_v2(data_vol, relaxation_type, Echo, Cutoff, varargin)
%
% multi_comp_fit(data_file_name, relaxation_type, ...
%               ['ROI', roi_vol, 'tissue', mask_vol])
%**************************************************************************
% DESCRIPTION:
...
%**************************************************************************
% EXAMPLE USES:
% multi_comp_fit(vol, 'T2', 'tissue',Echo, Cutoff, cls_mask)
...
%COM % [fid, message] = fopen(data_file_name,'r');
% if(fid == -1)
...
%COM if nargin < 5
    ...
    for counter = 2:2:(mask_opts)
        switch varargin{counter-1}
            case{'ROI'}
%COM                 roi_file_name = varargin{counter};
...
%                 end
            case{'tissue'}
%COM                 cls_file_name = varargin{counter};
%                 % check existence of mask file
...
%COM         [fid, message] = fopen(max_echoes_fname,'r');
%         if(fid == -1)
...
%COM % [data_pathstr, data_name, data_ext] = fileparts(data_file_name);
% 
...
%COM     ss_analysis = input('If single slice analysis is desired, enter the slice number. Otherwise, enter 0: ');
    ss_analysis = 0;
    if ss_analysis ~= 0
...
complex_analysis = 0;
fit_loop = 1;
%COM phase_fname = input('If complex data analysis is desired, enter phase data file name (otherwise, press "Enter": ');
phase_fname = '';
...
if ROI_flag == 1    
    % Open mask file
%COM     [roi_pathstr, roi_name, roi_ext] = fileparts(roi_file_name);
%     
...
%COM     [cls_pathstr, cls_name, cls_ext] = fileparts(cls_file_name);
%     
...
%COM echo_times = calc_echo_times(num_echoes);

echo_times = calc_echo_times_v2(num_echoes,Echo);
...
%COM        upper_cutoff_MW = input('Enter the desired upper cutoff for myelin water (in ms): ');
...
if (ROI_flag == 1 && voxelwise_ROI_flag == 0)
    ...
    if (ROI_flag == 1 && voxelwise_ROI_flag == 1)
        ...
        if (tissue_flag == 0 && voxelwise_ROI_flag == 0)
            ...
            % images!)        
%COM            create_brainmask(data_file_name);
            mask_flag = 1;
            % open brain mask
