...
TRs = linspace(15,100,18);

%%

MTsats = zeros(1,length(TRs))
MTRs = zeros(1,length(TRs))
T1s = zeros(1,length(TRs))
...
for ii=1:length(TRs)
    ...
    structHandler.legend = legend(string(TRs))
    hold on
end
xtickangle(45)
xlim([0 6])
...
%%
save("fig6_mtsat.mat", "TRs", "MTRs", "MTsats", "T1s")