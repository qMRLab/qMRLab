...
offsets = linspace(1000,10000,19);

%%

MTsats = zeros(1,length(offsets))
MTRs = zeros(1,length(offsets))
T1s = zeros(1,length(offsets))
...
for ii=1:length(offsets)
    ...
    structHandler.legend = legend(string(offsets))
    hold on
end
xtickangle(45)
xlim([0 6])
...
save("fig4_mtsat.mat", "offsets", "MTRs", "MTsats", "T1s")