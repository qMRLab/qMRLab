...
if moxunit_util_platform_is_octave
    % Turn off this warning, defeats the purpose otherwise
    eval('warning("off","Octave:num-to-str")');
    if exist('varargin','var')
        % Cast explicitly
        vargTxt = stringifyVarg(varargin);
        eval(['ocprintf(color,format,' vargTxt ')']);
    else
        ocprintf(color,format);
    end
    
else
    if strcmp(color,'orange'); color = '[1,0.5,0]'; end
    if exist('varargin','var')
        vargTxt = stringifyVarg(varargin);
        eval(['mcprintf(color,format,' vargTxt ')']);
    else
        mcprintf(color,format);
    end
    
end
end

function out = stringifyVarg(varg)
out = [];
for ii = 1:length(varg)
    
    cur = castVarargin(varg{ii});
    if ii~=length(varg)
        cur = [cur  ','];
    end
...
function out = castVarargin(varg)
    if isnumeric(varg)
        out = num2str(varg);
    elseif ischar(varg)
        out = ['''' varg ''''];
    elseif islogical(varg)
        chk = logical(varg);
        if chk; out = 'true'; else; out = 'false'; end
    end
...
function ocprintf (colorname, template, varargin)
...
switch forecolorname
    ...
    case "blue"
        forecolorcode = "34";
    case "orange"
        % Not really orange, just for matlab's sake.
        forecolorcode = "33";    
    case {"violet", "magenta"}
        forecolorcode = "35";
    case "cyan"
...
function count = mcprintf(style,format,varargin)
